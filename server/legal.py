...
def privacy_policy_body(operator_contact: str) -> str:
    ...
    <ul>
    <li>Sign-in information from the configured OIDC provider or Google, including the subject identifier and display name used to create the local account and browser session. Google login mode also retains the Google email address to display your calendar destination.</li>
    ...
    <li>For a one-shot export, a saved authenticated Phenikaa page or manually entered <code>userId</code> and <code>tokenJWT</code>. These values are used only for that request and are not added to a retained server session.</li>
    ...
    <li>Generated JSON and ICS calendar exports for each retained session, or temporary JSON, XLSX, and ICS files returned together for a one-shot export.</li>
    ...
    <p>In Google login mode, Google authenticates your app account and the same Google account is the calendar destination. The authorization requests openid, email, and profile in addition to the app-created Calendar scope. Calendar sync requires your permission; declining Calendar access does not prevent app login.</p>
    <p>The server uses OIDC data to authenticate you, Phenikaa session data to fetch your academic calendar, retained browser cookies to refresh retained Phenikaa access, and calendar records to create exports and dashboard status. One-shot export credentials and generated files are kept only while that request is processed. Google tokens are used only to perform the optional Google Calendar sync that you start by connecting Google.</p>
    ...
    <p>In Google login mode, encrypted Google authorization is retained at app-account level, including before you connect Phenikaa. Deleting a Phenikaa session does not delete this user-level grant. Pause calendar sync stops Google sync without revoking authorization, and signing out does not stop scheduled sync. Deleting the application account revokes Google access and removes the local grant, even if no Phenikaa session exists. Account deletion does not delete calendars or events already created in Google.</p>
    <p>Server state is retained while your account and session remain configured, including the SQLite database, generated exports, and browser profile. One-shot export credentials are not persisted, and its temporary files are removed after the ZIP response is prepared. Deleting a Phenikaa session from the dashboard removes that session's database row, generated exports, Google connection rows and event links through database cascading, and its retained browser profile. Disconnecting Google revokes the Google token when Google accepts the revocation request and removes the local Google connection. Logging out clears only the application session cookie in your browser.</p>
...
def terms_body(operator_contact: str) -> str:
    ...
    <p>Operators may instead enable Google login. In that mode, Google is both the app identity provider and the calendar destination; users separately connect Phenikaa. Identity scopes (openid, email, profile) are requested alongside app-created Calendar access. Calendar permission remains optional, and paused sync can be resumed without changing the login account.</p>
    <p>The service authenticates users through the configured OIDC provider. A user may create one retained Phenikaa calendar session, which captures the Phenikaa <code>userId</code> and <code>tokenJWT</code> after sign-in, stores the token encrypted, keeps browser cookies in a per-user session browser profile, and writes JSON and ICS exports. Alternatively, a user may provide saved authenticated page data or manual session values for a one-shot JSON, XLSX, and ICS export; those credentials are not retained and that flow does not start synchronization.</p>
