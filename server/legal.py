...
def privacy_policy_body(operator_contact: str) -> str:
    ...
    <p>The Google permission requested is exactly <code>https://www.googleapis.com/auth/calendar.events</code>. Sync is one way from Phenikaa to the connected user's Google primary calendar. The server creates new linked Google events, updates previously linked Google events, and deletes stale Google events only when they were previously linked by this app and disappeared from the selected Phenikaa range. It does not modify or delete unrelated Google Calendar events.</p>
...
def terms_body(operator_contact: str) -> str:
    ...
    <p>Google Calendar integration is optional. If connected, the service requests the <code>https://www.googleapis.com/auth/calendar.events</code> scope and performs one-way sync from Phenikaa to the connected user's Google primary calendar. It creates, updates, and deletes only Google events linked by this app; it does not provide two-way sync and does not manage unrelated Google events.</p>
